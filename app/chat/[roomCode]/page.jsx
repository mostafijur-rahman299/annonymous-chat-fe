...
import { Users, Send, LogOut, Hash, X, MessageCircle } from 'lucide-react'
import { Avatar, AvatarFallback, AvatarImage } from "@/components/ui/avatar"
import { Tooltip, TooltipContent, TooltipProvider, TooltipTrigger } from "@/components/ui/tooltip"

export default function ChatRoom() {
  const [messages, setMessages] = useState([
    { id: '1', sender: 'Alice', content: 'Hey everyone! How\'s it going?', timestamp: '2023-05-01T10:00:00Z' },
    { id: '2', sender: 'Bob', content: 'Hi Alice! All good here. How about you?', timestamp: '2023-05-01T10:02:00Z' },
    { id: '3', sender: 'Charlie', content: 'Hello folks! Just joined the room.', timestamp: '2023-05-01T10:05:00Z' },
    { id: '4', sender: 'Alice', content: 'Welcome Charlie! We were just getting started.', timestamp: '2023-05-01T10:06:00Z' },
    { id: '5', sender: 'Bob', content: 'Yeah, welcome! Any topics you\'d like to discuss?', timestamp: '2023-05-01T10:08:00Z' },
  ])
  const [input, setInput] = useState('')
  const [nickname, setNickname] = useState('')
  const [showExitDialog, setShowExitDialog] = useState(false)
  const [members, setMembers] = useState(['Alice', 'Bob', 'Charlie', 'David', 'Eva'])
  ...
  useEffect(() => {
    ...
    const handleClickOutside = (event) => {
      if (sidebarRef.current && !sidebarRef.current.contains(event.target) && showMembersMobile) {
  ...
  const MembersList = () => (
    <div className="h-full">
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-xl font-bold text-gray-800 dark:text-gray-200">Members ({members.length})</h2>
        <Button
          variant="ghost"
          size="icon"
          ...
          {members.map((member, index) => (
            ...
            >
              <Avatar className="h-8 w-8 mr-2">
                <AvatarImage src={`https://api.dicebear.com/6.x/initials/svg?seed=${member}`} />
  ...
  return (
    <div className="flex h-screen bg-gradient-to-br from-indigo-100 to-purple-100 dark:from-gray-900 dark:to-indigo-900 overflow-hidden">
      {/* Left Sidebar - Fixed on desktop/tablet, sliding on mobile */}
      <aside className="hidden md:block w-64 lg:w-80 bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm border-r border-gray-200 dark:border-gray-700">
      ...
      <div className="flex-1 flex flex-col">
          ...
          <div className="flex justify-between items-center">
            <div className="flex items-center">
              ...
              <div>
                ...
                <p className="text-sm text-purple-200">Room Code: {params.roomCode}</p>
              </div>
            </div>
          ...
          {messages.map((message) => (
            ...
            >
              <div className="flex items-start">
                ...
                <div>
                  <p className={`font-bold ${message.sender === nickname ? 'text-white' : 'text-purple-600 dark:text-purple-400'}`}>
                    {message.sender}
                  </p>
                  <p className={message.sender === nickname ? 'text-white' : 'text-gray-800 dark:text-gray-200'}>
                    {message.content}
                  </p>
                  <p className={`text-xs ${message.sender === nickname ? 'text-white/80' : 'text-gray-500 dark:text-gray-400'} mt-1`}>
        ...
        <div className="p-4 bg-white/50 dark:bg-gray-800/50 backdrop-blur-sm">
          <div className="flex space-x-2">
            ...
            />
            <Button onClick={sendMessage} className="bg-purple-600 hover:bg-purple-700 text-white transition-colors duration-200">
        ...
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction onClick={() => {
              ...
              router.push('/')
            }} className="bg-red-500 hover:bg-red-600 text-white">Exit</AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
    </div>
